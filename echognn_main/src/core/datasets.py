...
class EchoNetEfDataset(Dataset, ABC):
    ...
    def __init__(self,
        ...
        print("chnage csv file path in the training ")
        # filelist_path = '/scratch/st-puranga-1/users/bassant/code/MEDSAM2/MedSAM2/toy.csv'
        filelist_path = '/arc/project/st-puranga-1/datasets/echonet/FileList.csv'

        filelist_df = pd.read_csv(filelist_path)
        
        filelist_df = filelist_df.dropna(subset=['ESFrame', 'EDFrame'])
        ...
        mea_f = (mean, mean, mean)  # Example values for RGB channels
        std_f= (std, std, std)
        ...
        self.upsample = None
        if zoom_aug:
            self.upsample = Upsample(size=(256, 256), mode='nearest')

        # Other attributes
        self.num_frames = num_frames
        self.num_vids_per_sample = 1
        self.num_clips_per_vid = num_clips_per_vid
        self.num_frames_per_cycle = num_frames
    ...
    def __getitem__(self, idx: int) -> torch_geometric.data.data:
        ...
        # Get the video
        # dynamic (channels=3, frames, height, width)
        cine_vid = self._loadvideo(self.patient_data_dirs[idx]) # H,W,3,F

        # Transform video
        # edit_here
        # cine_vid = self.trans(cine_vid)
        # print(cine_vid.shape)  # (F, 3, H, W)
        
        # Applying the transform to each frame
        cine_vid_transformed = []
        for i in range(cine_vid.shape[3]):  # Iterate over frames (F)
            frame = cine_vid[:, :, :, i]  # Get the frame of shape (H, W, 3)
            transformed_frame = self.trans(frame)  # Apply ToTensor() and Normalize
            cine_vid_transformed.append(transformed_frame)

        # Stack frames into a 4D tensor (F, 3, H, W)
        cine_vid = torch.stack(cine_vid_transformed, dim=0)  # Shape: (F, 3, H, W)


        # modified
        # transformed_frames = [self.trans(cine_vid[:, i, :, :].transpose(1, 2, 0)) for i in range(Frames)]

        # Stack frames into a 4D tensor (F, 3, H, W)
        # transformed_frames C,H,W  > C,F,H,W
        ...
        if (idx in self.train_idx) and self.zoom_aug:
            if np.random.randint(0, 2):
                h, w = cine_vid.shape[-2], cine_vid.shape[-1]

                # Ensure cropping range is valid
                if h < 90 or w < 92:
                    print(f"Warning: Skipping zoom_aug due to small input size {cine_vid.shape}")
                else:
                    cine_vid = cine_vid[:,:, 0:90, 20:92]  
                    cine_vid = self.upsample(cine_vid)
                    
        # cine_vid = cine_vid.permute(1,0,2,3)
        ...
        # Interpolate if needed
         #  edit_here
        cine_vid =cine_vid.squeeze(1)
        cine_vid = cine_vid.permute(0, 2, 1, 3, 4)
        
        if cine_vid.shape[2] < self.num_frames:
            missing_frames = torch.zeros(cine_vid.shape[0], 3, self.num_frames - cine_vid.shape[2], 256, 256)
            cine_vid = torch.cat((cine_vid, missing_frames), dim=2)
    ...
    def extract_train_data(self, cine_vid: torch.tensor) -> (torch.tensor, np.ndarray):
        ...
        if self.num_frames > video_num_frames:
            ...
            # cine_vid = cine_vid[list(range(0, video_num_frames))].unsqueeze(0)
            frame_idx = np.arange(start=0, stop=video_num_frames)
        ...
        # take num_frames from the whole range of the video if number of frame per cycle is larger than video length
            ...
            # cine_vid = cine_vid[frame_idx].unsqueeze(0)
            if self.num_clips_per_vid > 1:
                cine_vid = torch.tensor(np.vstack([cine_vid] * self.num_clips_per_vid))
                frame_idx = np.vstack([frame_idx] * self.num_clips_per_vid)
        ...
        # Follow the procedure outlined in the paper
            ...
            # cine_vid_prim = cine_vid[frame_idx_prim].unsqueeze(0)
            ...
            for i in range(1, self.num_clips_per_vid):
                ...
                # cine_vid_temp = cine_vid[frame_idx_temp].unsqueeze(0)
...
class PretrainEchoNetEfDataset(Dataset, ABC):
    ...
    def __init__(self,
        ...
        # CSV file containing file names and labels
        filelist_path = '/arc/project/st-puranga-1/datasets/echonet/FileList.csv'
        # '/scratch/st-puranga-1/users/bassant/code/MEDSAM2/MedSAM2/toy.csv'
        # '/arc/project/st-puranga-1/datasets/echonet/FileList.csv'
    ...
    def __getitem__(self, idx: int) -> torch_geometric.data.data:
        ...
        # cine_vid =cine_vid.squeeze(1) #  (B, F, 3, H, W)
        
        # print(f"before cine_vid : {cine_vid.shape}")
        # cine_vid =cine_vid.squeeze(1)
        cine_vid = cine_vid.permute(0, 2, 1, 3, 4)  #  (B, F, 3, H, W) > #  (B, 3, F, H, W)
        # print(f"after cine_vid : {cine_vid.shape}")
...
#     data_config = {
#         # Path to dataset
#         "dataset_path": "/arc/project/st-puranga-1/datasets/echonet/",
#         # Name of the dataset
#         # The mean and std for the EchoNet-Dynamic dataset used to standardize data
...
#     dataset = dict()
#     dataset['train'] =PretrainEchoNetEfDataset(**data_config)
#     dataset['val'] =PretrainEchoNetEfDataset(**data_config)
#     dataloaders = {}
...
#     data_ = iter(dataloaders['train'])
#     batch = next(data_)  # Get a single batch from the loader

#     # Now, access its attributes:
#     edge_index = batch.edge_index
#     num_nodes = batch.num_nodes
#     x = batch.x

#     print(edge_index.shape)  # Should be [2, num_edges]
#     print(num_nodes)         # Should be an integer
#     print(x.shape)           # Should be [num_nodes, ...] (depends on your data format)
...
#     }
#     dataset_train = dict()
#     dataset_train['train'] =EchoNetEfDataset(**t_data_config)
#     dataset_train['val'] =EchoNetEfDataset(**t_data_config)
#     dataloaders_train = {}
#     sampler = SubsetRandomSampler(np.arange(start=0, stop=len(dataset_train['train'])))
#     dataloaders_train['train'] = DataLoader(dataset_train['train'],
#                                         batch_size=10,
#                                         drop_last=True,
#                                         sampler=sampler,
#                                         num_workers=1,
...
#     data_t = iter(dataloaders_train['train'])
#     batch_t = next(data_t)  # Get a single batch from the loader

#     # Now, access its attributes:
#     edge_index_t = batch_t.edge_index
#     num_nodes_t = batch_t.num_nodes
#     x_t = batch_t.x

#     print(edge_index_t.shape)  # Should be [2, num_edges]
#     print(num_nodes_t)         # Should be an integer
#     print(x_t.shape)           # Should be [num_nodes, ...] (depends on your data format)
